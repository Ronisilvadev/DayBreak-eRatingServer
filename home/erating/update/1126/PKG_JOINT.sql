...
CREATE PROCEDURE appStoreCharge(
    in  s_Charge_Detail_ID            varchar(128),
    in  s_User_Name                   varchar(32),
    in  n_Game_ID                     integer,
    in  n_Gateway_ID                  integer,
    in  s_Product_ID                  varchar(128),
    in  n_Quantity                    integer,
    in  s_App_Item_ID                 varchar(128),
    in  s_Transcation_ID              varchar(128),
    in  s_Purchase_Date               varchar(128),
    in  s_Purchase_Date_Ms            varchar(128),
    in  s_Purchase_Date_Pst           varchar(128),
    in  s_Original_Transaction_ID     varchar(128),
    in  s_Oiginal_Purchase_Date       varchar(128),
    in  s_Original_Purchase_Date_Ms   varchar(128),
    in  s_Original_Purchase_Date_Pst  varchar(128),
    in  s_Unique_Identifier           varchar(128),
    in  s_Bid                         varchar(128),
    in  s_Bvrs                        varchar(128),
    in  s_Server_Ip                   varchar(32),
    in  s_Client_Ip                   varchar(32),
    in  s_UserId                      varchar(32),
...
BEGIN
    ...
    declare  n_Tmp_Charge_Amout        integer;
    declare  n_Tmp_Charge_Money        integer;
    declare  n_Tmp_Charge_Detail_ID    bigint;
    declare  f_Tmp_Product_Price       float;

    label_proc:begin
        set n_Result = 0;

        if(
            (s_Charge_Detail_ID            is null) or
            (s_User_Name                   is null) or
            (n_Game_ID                     is null) or
            (n_Gateway_ID                  is null) or
            (s_Product_ID                  is null) or
            (n_Quantity                    is null) or
            (s_App_Item_ID                 is null) or
            (s_Transcation_ID              is null) or
            (s_Purchase_Date               is null) or
            (s_Purchase_Date_Ms            is null) or
            (s_Purchase_Date_Pst           is null) or
            (s_Original_Transaction_ID     is null) or
            (s_Oiginal_Purchase_Date       is null) or
            (s_Original_Purchase_Date_Ms   is null) or
            (s_Original_Purchase_Date_Pst  is null) or
            (s_Unique_Identifier           is null) or
            (s_Bid                         is null) or
            (s_Bvrs                        is null) or
            (s_Server_Ip                   is null) or
            (s_Client_Ip                   is null) or
        ...
        select
            charge_amount,      product_price
        into 
            n_Tmp_Charge_Amout, f_Tmp_Product_Price
        from
            sys_appstore_product
        where
            game_id = n_Game_ID
        and
        ...
        set n_Tmp_Charge_Ratio = CAST(s_Tmp_Charge_Ratio AS signed);
        set n_Tmp_Charge_Detail_ID = CAST(s_Charge_Detail_ID AS unsigned);

        set n_Tmp_Charge_Money = n_Tmp_Charge_Amout/n_Tmp_Charge_Ratio;

        insert ignore into log_appstore_charge(
            charge_detail_id,       user_name,           game_id,             gateway_id,
            product_id,             quantity,            app_item_id,         transaction_id,
            purchase_date,          purchase_date_ms,    purchase_date_pst,   original_transaction_id,
            original_purchase_date, original_purchase_date_ms, original_purchase_date_pst,
            unique_identifier,      bid,                 bvrs,                operate_time,
            charge_amount,          product_price,       all_charge_amount,   all_product_price
        )
        values (
            s_Charge_Detail_ID,     s_User_Name,         n_Game_ID,           n_Gateway_ID,
            s_Product_ID,           n_Quantity,          s_App_Item_ID,       s_Transcation_ID,
            s_Purchase_Date,        s_Purchase_Date_Ms,  s_Purchase_Date_Pst, s_Original_Transaction_ID,
            s_Oiginal_Purchase_Date,s_Original_Purchase_Date_Ms, s_Original_Purchase_Date_Pst,
            s_Unique_Identifier,    s_Bid,               s_Bvrs,              now(),
            n_Tmp_Charge_Amout,     f_Tmp_Product_Price, n_Tmp_Charge_Amout*n_Quantity, f_Tmp_Product_Price*n_Quantity
        );
        ...
        call PfCharge1(
                       ...
                       s_User_Name,
                       n_Gateway_ID,
                       n_Tmp_Charge_Money*n_Quantity,
                       n_Tmp_Charge_Amout*n_Quantity,
                       d_Charge_Time,
                       s_Server_Ip,
                       s_Client_Ip,
                       s_Charge_Detail_ID,
