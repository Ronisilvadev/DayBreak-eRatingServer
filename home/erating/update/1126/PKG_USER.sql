...
DROP PROCEDURE authenticateEx3;
...
DROP PROCEDURE recordAppMsgPush;
DELIMITER $$
...
BEGIN
    declare n_Ret_User_ID            integer unsigned default  0;    /* PKG_UTIL.E_ERROR */
...
BEGIN
    ...
    declare s_Tmp_ID_Card        varchar(32);
    label_proc:begin
        ...
        if (FOUND_ROWS() <> 0) then
            ...
            if (
                 (s_ID_Card IS NULL ) OR
                 (s_ID_Card = '')
                )
            then
                set s_Tmp_ID_Card = n_Ret_User_ID;
            else
                set s_Tmp_ID_Card = s_ID_Card;
            end if; 
            ...
            then
                set s_Tmp_ID_Card = n_Tmp_User_ID;
            else
                set s_Tmp_ID_Card = s_ID_Card;
            end if;
...
CREATE PROCEDURE authenticateEx3(
 IN  n_Game_ID              INTEGER, 
 IN  n_Gateway_ID           VARCHAR(255),
 IN  s_User_Name            VARCHAR(255),
 IN  s_Password             VARCHAR(255),
...
CREATE PROCEDURE recordAppMsgPush(
    in   n_Game_ID          integer,
    in   n_User_ID          integer unsigned,
    in   n_Client_Type      integer,
    in   s_App_Rgist_ID     varchar(256),
...
BEGIN
    ...
    label_proc:begin
        ...
        begin
            insert into UMS_USER_APP_MSG
            (game_id, user_id, client_type, app_regist_id)
            values
            (n_Game_ID , n_User_ID, n_Client_Type, s_App_Rgist_ID)
            on duplicate key
            update app_regist_id = s_App_Rgist_ID;
