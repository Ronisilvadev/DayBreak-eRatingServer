...
DROP PROCEDURE FillChargeRealActivity;
DROP PROCEDURE dailyAccumChargeActivity;
DROP PROCEDURE dailyIntervalChargeActivity;
...
BEGIN
    ...
    declare n_Tmp_Activity_State    integer;
    declare n_Tmp_Activity_Type     integer;
    declare n_Tmp_Limit_Type        integer default 0;
...
BEGIN
    ...
    declare n_Tmp_Counter           integer default 0;
    declare n_Stop                  integer;
    declare n_cur_Game_ID           integer;
    declare cur_Game_ID_List cursor for
        select
...
BEGIN
    declare n_RetCode           integer default 0;
    declare n_Last_User_ID      integer unsigned;
...
CREATE PROCEDURE FillChargeRealActivity(
IN  n_Game_ID        INTEGER, 
IN  s_User_Name      VARCHAR(255), 
IN  n_Gateway_ID     INTEGER,
IN  n_Charge_Amount  INTEGER, 
...
BEGIN
    ...
    declare n_Tmp_Package_ID            integer;
    declare n_Tmp_User_ID               integer unsigned;
    declare n_Tmp_upper_limit           integer;
    declare n_Tmp_sum_amount            integer;
    declare n_Tmp_before_amount         integer;
    declare n_Tmp_charge_amount         integer;
    declare n_Tmp_item_amount           integer;
    declare n_cur_Activity_ID           integer;
    declare n_cur_Flag_Bit              integer;
    declare d_cur_Gift_Bgn_Time         datetime;
    declare d_cur_Begin_Time            datetime;
    ...
    label_proc:begin
        ...
        label_loop:loop
            set n_Stop = 0;
            fetch cur_Activity_List into n_cur_Activity_ID, n_cur_Flag_Bit, d_cur_Gift_Bgn_Time, n_cur_effect_period, d_cur_Begin_Time,d_cur_End_Time,
            b_cur_mask;
            if (n_Stop = 1) then
                leave label_loop;
            end if;
            ...
            select
                upper_limit, package_id
            into
                n_Tmp_upper_limit, n_Tmp_Package_ID
            from
                SYS_ACTIVITY_DETAIL
            where
                activity_id  = n_cur_Activity_ID  and
            ...
            where
                ...
                subject_id = 3 and
                charge_type = 1 and
                rating_id = if(n_Tmp_Gateway_ID=0, rating_id, n_Tmp_Gateway_ID) and
                charge_time >= if (date(now())>date(d_cur_Begin_Time),date(now()),d_cur_Begin_Time) and
                charge_time <= if (date(now())<date(d_cur_End_Time),date(adddate(now(),1)),d_cur_End_Time);

            set n_Tmp_before_amount = n_Tmp_sum_amount - n_Charge_Amount;
            if( n_Tmp_before_amount < 0 ) then
                set n_Result = -1470;
                leave label_loop;
            end if;
            set n_Tmp_charge_amount = n_Tmp_before_amount - (floor(n_Tmp_before_amount/n_Tmp_upper_limit))*n_Tmp_upper_limit + n_Charge_Amount;
            set n_Tmp_item_amount = floor(n_Tmp_charge_amount/n_Tmp_upper_limit);

            if(n_Tmp_item_amount > 0 ) then
                call sendChargeGifts(
                        n_Game_ID,
                        n_Tmp_User_ID,          -- user_id
                        0,                      -- role_id
...
CREATE PROCEDURE dailyIntervalChargeActivity(
IN  n_Game_ID        INTEGER,
IN  s_User_Name      VARCHAR(255),
IN  n_Gateway_ID     INTEGER,
IN  n_Charge_Amount  INTEGER,
...
BEGIN
    declare n_RetCode                   integer  default 0;
    declare n_Tmp_User_ID               integer  unsigned;
    declare n_Tmp_max_lower_limit       integer;
    declare n_Tmp_sum_amount            integer;
    declare n_Tmp_before_amount         integer;
    declare n_cur_Activity_ID           integer;
    declare n_cur_Package_ID            integer;
    ...
    declare n_Tmp_Gateway_ID            integer;
    declare n_Stop                      integer;
    declare n_Tmp_Count_Day             integer default 0;
    ...
    declare cur_Activity_List cursor for
        ...
        from
            SYS_ACTIVITY_INFO
        where
            activity_type = 16 and
            activity_state = 1 and
            end_time > now() and
            begin_time <= now() and
            game_id = n_Game_ID;

    declare cur_Package_List cursor for
        select
            package_id
        from
            SYS_ACTIVITY_DETAIL
        where
            activity_id  = n_cur_Activity_ID  and
            game_id      = n_Game_ID and
            lower_limit <= n_Tmp_sum_amount and
            package_id not in
            (
                select
                    package_id
                ...
                where
                    activity_id  = n_cur_Activity_ID  and
                    game_id      = n_Game_ID and
                    lower_limit <= n_Tmp_before_amount and
                    n_Tmp_before_amount <> 0
            );
    ...
    label_proc:begin
        ...
        label_loop:loop
            ...
            select
                max(lower_limit)
            into
                n_Tmp_max_lower_limit
            from
                SYS_ACTIVITY_DETAIL
            where
                activity_id  = n_cur_Activity_ID  and
                game_id      = n_Game_ID;
            if(n_Tmp_max_lower_limit is null) then
                set n_Result = -1518;
                iterate label_loop;
            end if;
            ...
            IF (NOW()>(d_cur_Begin_Time + INTERVAL (DATE(NOW())-DATE(d_cur_Begin_Time)) DAY)) THEN
                SET n_Tmp_Count_Day = DATE(NOW())-DATE(d_cur_Begin_Time);
            ELSE
                SET n_Tmp_Count_Day = DATE(NOW())-DATE(d_cur_Begin_Time) - 1;
            END IF;
            ...
            where
                ...
                charge_type = 1 and
                rating_id = if(n_Tmp_Gateway_ID=0, rating_id, n_Tmp_Gateway_ID) and
                charge_time >= IF (DATE(NOW())>DATE(d_cur_Begin_Time),d_cur_Begin_Time + INTERVAL n_Tmp_Count_Day DAY,d_cur_Begin_Time) AND
                charge_time <= IF (DATE(NOW())<DATE(d_cur_End_Time),d_cur_Begin_Time + INTERVAL (n_Tmp_Count_Day + 1)  DAY,d_cur_End_Time);

            set n_Tmp_before_amount = n_Tmp_sum_amount - n_Charge_Amount;
            if( n_Tmp_before_amount < 0 ) then
                set n_Result = -1470;
                iterate label_loop;
            elseif( n_Tmp_before_amount >= n_Tmp_max_lower_limit ) then
                set n_Result = 1;
                iterate label_loop;
            else
                open cur_Package_List;
                label_Package_loop:loop
                    set n_Stop = 0;
                    fetch cur_Package_List into n_cur_Package_ID;

                    if (n_Stop = 1) then
                        leave label_Package_loop;
                    end if;
                ...
                end loop label_Package_loop;
                close cur_Package_List;
            end if;
        end loop label_loop;
        close cur_Activity_List;
...
CREATE PROCEDURE AllChargeRealTime(
IN  n_Game_ID        INTEGER, 
IN  s_User_Name      VARCHAR(255), 
IN  n_Gateway_ID     INTEGER,
IN  n_Charge_Amount  INTEGER, 
...
BEGIN
    ...
    label_proc:begin
        ...
        call dailyIntervalChargeActivity(
              n_Game_ID,
              s_User_Name,    
              n_Gateway_ID,    
              n_Charge_Amount,
...
CREATE PROCEDURE ChargeRealTime(
IN  n_Game_ID        INTEGER, 
IN  s_User_Name      VARCHAR(255), 
IN  n_Gateway_ID     INTEGER,
IN  n_Charge_Amount  INTEGER, 
...
CREATE PROCEDURE getActivityConsumeDetail(
in  n_activityId    integer,
in  n_gameId        integer
)
BEGIN
    label_proc:begin
        ...
        SELECT
        upper_limit, lower_limit, package_id, limit_type, subject_id, product_code
...
BEGIN
    declare n_RetCode                  integer default 0;
    declare n_Stop                     integer default 0;
    declare n_Stop_gateway             integer default 0;
    ...
    declare d_cur_end_time                 datetime;
    declare b_cur_mask                     tinyblob;
    declare n_cur_gateway_id               integer;/*add by Liushan 2013-01-10*/
    ...
    label_proc:begin
        ...
        label_loop:loop
            ...
            begin
                /*add by LiuShan 2013-01-10 */       
                declare activity_Gateway_List cursor for
                select
                     gateway_id
                from 
                     SYS_ACTIVITY_GATEWAY
                where
                     activity_id = n_cur_activity_id and
                     game_id     = n_cur_game_id;
                declare continue handler for not found set n_Stop_gateway = 1;
                /*add by LiuShan 2013-01-10 */ 
               
                open activity_Gateway_List;
                label_gateway_loop:loop
                
                    fetch activity_Gateway_List into n_cur_gateway_id;
                
                    if (n_Stop_gateway = 1) then
                        leave label_gateway_loop;
                    end if;
                    ...
                    if(n_Limit_Type = 1) then
                        call sendItemCountGifts(
                                                n_cur_game_id,
                                                n_cur_activity_id,
                                                n_cur_gateway_id,/*add by LiuShan 2013-01-11*/
                                                6,
                                                n_cur_flag_bit,
                                                d_Tmp_Gift_Bgn_Time,
                                                date_add(d_Tmp_Gift_Bgn_Time, interval n_cur_effect_period second),
...
(
    in  n_Game_ID            integer,
    in  n_Activity_ID        integer,
    in  n_Gateway_ID         integer,/*add by LiuShan 2013-01-11*/
    in  n_Activity_Type      integer,
    in  n_Flag_Bit           integer,
    in  d_Gift_Bgn_Time      datetime,
    in  d_Gift_End_Time      datetime,
...
BEGIN
    ...
    declare n_Last_User_ID       integer unsigned default 0;
    declare n_Temp_User_ID       integer unsigned;
    declare n_Temp_Package_ID    integer;

    -- 单一物品指定服区 累计数量
    declare cur_Single_List_SingleGateway cursor for
        select
            A.user_id, B.package_id
        from
        (
    ...
    -- 单一物品全服区 累计数量
    declare cur_Single_List_AllGateway cursor for
        select
            A.user_id, B.package_id
        from
        (
    ...
    -- 所有物品指定服区 累计数量
    declare cur_All_List_SingleGateway cursor for
        select
            A.user_id, B.package_id
        from
        (
    ...
    -- 所有物品全服区 累计数量
    declare cur_All_List_AllGateway cursor for
        select
            A.user_id, B.package_id
        from
        (
    ...
    label_proc:begin
        ...
        if(n_Gateway_ID = 0) then
                begin
                    open cur_Single_List_AllGateway;

                    label_loop:loop
                        fetch cur_Single_List_AllGateway into n_Temp_User_ID, n_Temp_Package_ID;
                        if( n_Stop = 1) then
                            leave label_loop;
                        end if;
                            ...
                            if(n_Last_User_ID <> n_Temp_User_ID) then
                                ...
                                if(n_RetCode <> 1) then
                                    if(n_RetCode = -1414) then
                                        ...
                                        set n_Result = n_RetCode;
                                        close cur_Single_List_AllGateway;
                                        leave label_proc;
                                    end if;
                                end if;
                            end if;
                    ...
                    label_loop:loop
                        ...
                        call clearActivityFlag(n_Game_ID, n_Temp_User_ID, n_Gateway_ID, n_Flag_Bit, n_RetCode);
                ...
                begin
                    ...
                    set n_Stop = 0;
                    open cur_All_List_AllGateway;

                    label_loop:loop
                        fetch cur_All_List_AllGateway into n_Temp_User_ID, n_Temp_Package_ID;
                        if( n_Stop = 1) then
                            leave label_loop;
                        end if;
                ...
                begin
                    open cur_Single_List_SingleGateway;

                    label_loop:loop
                        fetch cur_Single_List_SingleGateway into n_Temp_User_ID, n_Temp_Package_ID;
                        if( n_Stop = 1) then
                            leave label_loop;
                        end if;
                            ...
                            if(n_Last_User_ID <> n_Temp_User_ID) then
                                ...
                                if(n_RetCode <> 1) then
                                    if(n_RetCode = -1414) then
                                        ...
                                        set n_Result = n_RetCode;
                                        close cur_Single_List_SingleGateway;
                                        leave label_proc;
                                    end if;
                                end if;
                            end if;
                ...
                begin
                    ...
                    set n_Stop = 0;
                    open cur_All_List_SingleGateway;

                    label_loop:loop
                        fetch cur_All_List_SingleGateway into n_Temp_User_ID, n_Temp_Package_ID;
                        if( n_Stop = 1) then
                            leave label_loop;
                        end if;
                            ...
                            if(n_Last_User_ID <> n_Temp_User_ID) then
                                ...
                                if(n_RetCode <> 1) then
                                    if(n_RetCode = -1414) then
                                        ...
                                        set n_Result = n_RetCode;
                                        close cur_All_List_SingleGateway;
                                        leave label_proc;
                                    end if;
                                end if;
                            end if;
