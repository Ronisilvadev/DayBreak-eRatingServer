...
{ 
   ...
   var drawed = (area.childNodes.length>0);
	...
	document.location.href="showResourceRelation.do?resId=" + id;
};
...
treeCheckBox.prototype.selectParent = function( cid )
{
	...
	while(cid!=this.topid)
	{
		...
		var o = document.getElementById(this.name+"_node_radio_"+cid);
		o.checked = true;
...
treeCheckBox.prototype.toString = function( containObj )
{
	if( !this.drawed ){
	 ...
	 var cid = this.topid;
	 if(this.useCheckBox){
			var checkBox = "<input type='radio'  id='"+this.name+"_node_radio_"+cid+"' ";
	...
	for (var cidKey in pIndex)
	{
		...
		var cText=this.rows[cid]['title'];
		 if(this.useCheckBox){
				...
				checkBox +=" name='"+this.checkBoxName+"' value='"+cid+"' title='"+cText+"' />";
		 }else{
			 var checkBox = "";
		 }
		//alert(cText);break;
...
str +="</span>";
str +="<span id='tree_node_text_1' class='TreeText'>radio. a href..</span>";
str +="</NOBR>";
str +="</div>";
