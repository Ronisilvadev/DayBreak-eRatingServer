...
<html>
	...
	<body bgcolor="#ffffff">
		...
		<form name="pageForm" action="${requestScope.operType }.do" method="post">
			<table width="90%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr height="30px">
					<td colspan="7" align="left">
						<gmweb:createCompenTitleButton operType="${requestScope.operType}" actionName="roleCompensate" methodType="AddItemCompensate" />
					</td>
				</tr>
				<tr>
				...
				<c:forEach items="${requestScope.pageInfo.infoList}" var="AddItemCompensate" varStatus="status">
					<tr height="23px">
						<td width="10%" align="center" class="t_border_l">
							<input name="compensateId" type="checkbox" value="${AddItemCompensate.COMPENSATE_ID }" onclick="uncheckAll(document.forms[0]);" <c:if test="${requestScope.apprType != 'batchFirstApprAddItemCompensate' and requestScope.apprType != 'batchSecondApprAddItemCompensate' }">disabled</c:if> />
						</td>
						<td align="center" class="t_border_l">
							${status.index + 1 }
						</td>
						<td align="center" class="t_border_l">
							${AddItemCompensate.OPERATOR_NAME }
						</td>
						<td align="center" class="t_border_l">
							<fmt:formatDate value="${AddItemCompensate.INSERT_TIME }"
								pattern="yyyy-MM-dd HH:mm:ss" />
						</td>
						<td align="center" class="t_border_l">
							${AddItemCompensate.REASON }
						</td>
						<td align="center" class="t_border_l">
							<gmweb:approvalResult2Text approvalResult="${AddItemCompensate.APPROVAL_RESULT}" />
						</td>
						<td align="center" class="t_border_r">
							<gmweb:createCompenOperLink operType="${requestScope.operType}" actionName="roleCompensate" methodType="AddItemCompensate" compensateId="${AddItemCompensate.COMPENSATE_ID}"/>
						</td>
					</tr>
				</c:forEach>
				<tr height="30px">
			...
			<c:if test="${requestScope.operType == 'showAddItemCompensate' }">
			<tr height="30px">
				<td colspan="7" align="left">
					<input type="button" value="<bean:message bundle="pageResources" key="page.roleCompensate.awardCompensate.createAwardCompensate"/>" onclick="window.location.href='showCreateAddItemCompensate.do'" />
				</td>
			</tr>
			</c:if>
			<c:if test="${requestScope.apprType == 'batchFirstApprAddItemCompensate' or requestScope.apprType == 'batchSecondApprAddItemCompensate' }">
			<tr height="30px">
				<td colspan="7" align="left">
