...
<html>
  ...
  <body bgcolor="#ffffff">
  <form action="resourceRelation.do" method="post">
  <table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">
			<tr height="30px">
				<td align="center" colspan="2" ></td>
			</tr>
			<tr>
				<td align="center" colspan="2" class="t_head"><bean:message bundle="pageResources" key="page.purview.resourceRelation.title"/></td>
			</tr>
			<tr height="23px" class="h_CDE4F6">
				<td width="35%" align="center"><bean:message bundle="pageResources" key="page.purview.resource2Game.title2"/></td>
				<td align="center"><bean:message bundle="pageResources" key="page.purview.resource2Game.title3"/></td>
			</tr>
			<tr class="ffffff">
				<td class="t_border_l" valign="top">
					 <c:if test="${not empty requestScope.menuItemList }">
				  	<PRE>
						<div id=Tc></div>
					<SCRIPT language=javascript src="js/treeRadio.0.2.js" type=text/javascript></SCRIPT>
					<SCRIPT>
						var rows = new Object; 
						var rowsPidIndex = new Object; 
					  	rows = {
					  			<c:forEach var="menu" items="${menuItemList}" varStatus="status1">
					...
					<SCRIPT>
						var rtree = new  treeCheckBox( 'rtree'  , rows , rowsPidIndex ); 
						rtree.iconPath='images/tree2/';
						rtree.useCheckBox=true;
						rtree.checkBoxName='resId';
						rtree.checkBoxChecked='';
						rtree.toString( Tc ); 
					</SCRIPT>
					     ...
					     function  selectRadio(){
					         ...
					         for ( var  i = 0 ; i < radios.length; i ++ ){
					             ...
					             if (obj && obj.type && obj.type == "radio"){
					             	if (obj.id && obj.id == "rtree_node_radio_${requestScope.resId}"){
					             		obj.checked = true;
					             	}
					             }
					        }
				...
				<td class="t_border_r">
				  <c:if test="${not empty requestScope.allItemList }">
				  	<PRE>
						<div id=Tc2></div>
					<SCRIPT language=javascript src="js/treeCheckBox.0.2.modify.js" type=text/javascript></SCRIPT>
					<SCRIPT>
						var rows = new Object; 
						var rowsPidIndex = new Object; 
					  	rows = {
					  			<c:forEach var="menu" items="${allItemList}" varStatus="status1">
					  			<c:forEach var="menuItem" items="${menu.childMenuItem}" varStatus="status2">
							  	'${menuItem.id}' : {'id':'${menuItem.id}', 'pid':'${menuItem.parentId}', 'title':"<bean:message bundle="menuResources" key="${menuItem.name}"/>（<bean:message bundle="menuResources" key="${menuItem.desc }"/>）"}
								<c:if test="${!(status1.last && status2.last)}">
								,
