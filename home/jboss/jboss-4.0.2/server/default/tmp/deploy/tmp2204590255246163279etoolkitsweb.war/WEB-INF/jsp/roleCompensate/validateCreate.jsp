...
<body bgcolor="#ffffff">
	...
	<form action="<c:if test="${empty requestScope.isError }">saveAwardCompensate</c:if>
	...
	<div align="center">
		...
		<c:if test="${empty requestScope.isError }">
			<c:if test="${requestScope.commitFlag == '1' }">
				<input type="submit" value=" <bean:message bundle="pageResources" key="page.roleCompensate.validateCreate.commit"/> "/>
			</c:if>
			<c:if test="${requestScope.commitFlag != '1' }">
