...
<body bgcolor="#ffffff">
	<c:if test="${empty requestScope.roleLog }">
		<html:form action="roleExpLog.do" method="post" onsubmit="return validateRoleExpLogForm(this);">
		<table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">
			<tr>
				<td align="center" height="30PX"></td>
			</tr>
			<tr>
				<td align="center" class="t_head" colspan="2"><bean:message bundle="pageResources" key="page.playerQuery.roleExpOperate.title"/></td>
			</tr>
			<tr height="30px">
				<td width="35%" class="t_border_l" align="right"><bean:message bundle="pageResources" key="page.common.gatewayId"/>：
				 </td>
				 <td class="t_border_r" align="left">
					<html:select property="gatewayId" style="width:150px">
					     <c:forEach items = "${requestScope.gatewayList}" var = "gateway">
								<html:option value="${gateway.gatewayId }">${gateway.gatewayId} - ${gateway.gatewayName}</html:option>
					     </c:forEach>
					</html:select>
				 </td>
			</tr>
			<tr height="30px"><td width="35%" class="t_border_l" align="right"><bean:message bundle="pageResources" key="page.playerQuery.common.loggateway"/>：
				 </td>
				 <td class="t_border_r" align="left">
					<html:select property="loggatewayId" style="width:150px">
						<c:forEach items="${requestScope.gatewayList}" var="gateway">
							<html:option value="${gateway.gatewayId }">${gateway.gatewayId} - ${gateway.gatewayName}></html:option>
						</c:forEach>
					</html:select>
				 ...
				 <td class="t_border_r" align="left">
					<html:select property="opType">
						<c:forEach items="${requestScope.dicList }" var="dic">
							<html:option value="${dic.opValue }">${dic.opValue } - ${dic.opText }></html:option>
						</c:forEach>
					</html:select>
				 </td>
			</tr>
			<tr height="30px">
				<td width="35%" class="t_border_l" align="right"><bean:message bundle="pageResources" key="page.worldOperation.issueSettingExp.expType"/>：
				 </td>
				 <td class="t_border_r" align="left">
					<html:select property="expType">
						<c:forEach items="${requestScope.expList }" var="dic">
							<html:option value="${dic.opValue }">${dic.opValue } - ${dic.opText }></html:option>
						</c:forEach>
					</html:select>
				 </td>
			...
			<tr height="30px">
				<td width="35%" class="t_border_l" align="right"><bean:message bundle="pageResources" key="page.common.startDate"/>：
				 ...
				 <td class="t_border_r" align="left">
					<html:text property="startTime"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
				</td>
			</tr>
			<tr height="30px">
				<td width="35%" class="t_border_l" align="right"><bean:message bundle="pageResources" key="page.worldOperation.common.endTime"/>
				 ...
				 <td class="t_border_r" align="left">
					<html:text property="endTime" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
					<!-- 
	...
	<html:form action="roleExpLog.do" method="post" onsubmit="return validateRoleItemLogForm(this);">
		<table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">
			<tr>
				<td align="center" height="30PX"></td>
			</tr>
			<tr>
				<td align="center" class="t_head" colspan="20"><bean:message bundle="pageResources" key="page.playerQuery.roleExpOperate.title"/></td>
			</tr>
			<tr class="h_CDE4F6">
