...
<html>
	...
	<body bgcolor="#ffffff">
		...
		<form name="pageForm" action="${requestScope.operType }.do" method="post">
			<table width="90%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr height="30px">
					<td colspan="7" align="left">
						<gmweb:createCompenTitleButton operType="${requestScope.operType}" actionName="roleCompensate" methodType="GoldCompensate" />
					</td>
				</tr>
				<tr>
				...
				<c:forEach items="${requestScope.pageInfo.infoList}" var="goldCompensate" varStatus="status">
					<tr height="23px">
						<td width="10%" align="center" class="t_border_l">
							<input name="compensateId" type="checkbox" value="${goldCompensate.compensateId }" onclick="uncheckAll(document.forms[0]);" <c:if test="${requestScope.apprType != 'batchFirstApprGoldCompensate' and requestScope.apprType != 'batchSecondApprGoldCompensate' }">disabled</c:if> />
						</td>
						<td align="center" class="t_border_l">
							${status.index + 1 }
						</td>
						<td align="center" class="t_border_l">
							${goldCompensate.operatorName }
						</td>
						<td align="center" class="t_border_l">
							<fmt:formatDate value="${goldCompensate.insertTime }"
								pattern="yyyy-MM-dd HH:mm:ss" />
						</td>
						<td align="center" class="t_border_l">
							${goldCompensate.reason }
						</td>
						<td align="center" class="t_border_l">
							<gmweb:approvalResult2Text approvalResult="${goldCompensate.approvalResult}" />
						</td>
						<td align="center" class="t_border_r">
							<gmweb:createCompenOperLink operType="${requestScope.operType}" actionName="roleCompensate" methodType="GoldCompensate" compensateId="${goldCompensate.compensateId}"/>
						</td>
					</tr>
				</c:forEach>
				<tr height="30px">
			...
			<c:if test="${requestScope.operType == 'showGoldCompensate' }">
			<tr height="30px">
				<td colspan="7" align="left">
					<input type="button" value="<bean:message bundle="pageResources" key="page.roleCompensate.awardCompensate.createAwardCompensate"/>" onclick="window.location.href='showCreateGoldCompensate.do'" />
				</td>
			</tr>
			</c:if>
			<c:if test="${requestScope.apprType == 'batchFirstApprGoldCompensate' or requestScope.apprType == 'batchSecondApprGoldCompensate' }">
			<tr height="30px">
				<td colspan="7" align="left">
