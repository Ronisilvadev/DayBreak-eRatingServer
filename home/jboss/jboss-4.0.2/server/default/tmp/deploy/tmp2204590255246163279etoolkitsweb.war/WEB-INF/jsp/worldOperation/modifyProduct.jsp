...
<body bgcolor="#ffffff">
	<form action="modifyProduct.do" method="post" onsubmit="return check();">
		<table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">
			<tr>
				<td align="center" height="30PX" colspan="2"></td>
			</tr>
			<tr>
				<td align="center" class="t_head" colspan="2"><bean:message bundle="pageResources" key="page.worldOperation.product.update"/></td>
			</tr>
			<tr height="30px">
				...
				<td class="t_border_r" align="left">
				 	...
				 	<input name="productId" type="hidden" value="${requestScope.product.productId}" />
				 	<input name="productCode" type="hidden" value="${requestScope.product.productCode}" />
				 	<input name="productState" type="hidden" value="${requestScope.product.state}" />
				 </td>
			</tr>
			<tr height="30px">
				<td width="35%" class="t_border_l" align="right"><bean:message bundle="pageResources" key="page.worldOperation.productInfo.item_name"/>
				 ...
				 <td class="t_border_r" align="left">
				 	<input name="productName" type="text" value="${ requestScope.product.productName}" />
				 </td>
			</tr>
			<tr height="30px">
				...
				<td class="t_border_r" align="left">
				 	<select name="useType">
				 		<option value='1' 
				 			<c:if test="${requestScope.product.useType eq 1}">selected</c:if>
				        ><bean:message bundle="pageResources" key="page.worldOperation.addProduct.itemTypeValue1"/></option>
						<option value='2'
							<c:if test="${requestScope.product.useType eq 2}">selected</c:if>
						><bean:message bundle="pageResources" key="page.worldOperation.addProduct.itemTypeValue2"/></option>
						<option value='3' 
							<c:if test="${requestScope.product.useType eq 3}">selected</c:if>
						><bean:message bundle="pageResources" key="page.worldOperation.addProduct.itemTypeValue3"/></option>
						<option value='4'
							<c:if test="${requestScope.product.useType eq 4}">selected</c:if>
						><bean:message bundle="pageResources" key="page.worldOperation.addProduct.itemTypeValue4"/></option>
					</select>
				 </td>
			</tr>
			<tr height="30px">
				 ...
				 <td class="t_border_r" align="left">
				  	<textarea cols="20" rows="5" name="productDesc" >${ requestScope.product.productDesc}</textarea>
