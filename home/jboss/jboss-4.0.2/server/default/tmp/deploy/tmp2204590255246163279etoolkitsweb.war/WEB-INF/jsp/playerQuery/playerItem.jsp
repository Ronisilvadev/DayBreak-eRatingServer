...
<head>
	...
	<html:javascript formName="/getPlayerItems"/>
...
<body bgcolor="#ffffff">
	<c:if test="${empty requestScope.pageInfo }">
		<html:form action="getPlayerItems.do" method="post" onsubmit="return validateGetPlayerItemsForm(this);">
		<table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">
			<tr>
				<td align="left" height="30PX" colspan="2"></td>
			</tr>
			<tr>
				<td align="center" class="t_head" colspan="2"><bean:message bundle="pageResources" key="page.playerQuery.playerItem.title"/></td>
			</tr>
	...
	<c:if test="${not empty requestScope.pageInfo }">
		<table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">
			<tr height="30px">
				<td colspan="8" align="left"><b><bean:message bundle="pageResources" key="page.playerQuery.getplayerItem.oldTip"/></b></td>
			</tr>
			<tr>
				<td colspan="8" align="center" class="t_head"><bean:message bundle="pageResources" key="page.playerQuery.accountDetail.player"/> "${playerName }" <bean:message bundle="pageResources" key="page.playerQuery.playerItem.playerItem"/></td>
			</tr>
		...
		<c:forEach items="${requestScope.pageInfo.infoList}" var="playerItem">
			<tr height="23px">
					<td align="center" class="t_border_l">${playerItem.gatewayId }</td>
					<td align="center" class="t_border_l"  <c:if test="${playerItem.endTime.time < currentDate.time }">style="color:red;"</c:if>>${playerItem.activityId }</td>
					<td align="center" class="t_border_l">${playerItem.itemNum }</td>
					<td align="center" class="t_border_l"><fmt:formatDate value="${playerItem.beginTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td align="center" class="t_border_l"><fmt:formatDate value="${playerItem.endTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					<td align="center" class="t_border_l">${playerItem.playerItemCode }</td>
					<td align="center" class="t_border_r">
						<html:form action="modifyItemName.do" method="post" onsubmit="return validateModifyItemNameForm(this);">
							<input name="itemName" type="text" value="${playerItem.playerItemName }" />
							<input name="playerItemCode" type="hidden" value="${playerItem.playerItemCode }" />
							<input name="comeFromAction" type="hidden" value="/getPlayerItems.do?playerName=${playerName }&itemCode=${playerItem.playerItemCode  }&beginDate=<fmt:formatDate value="${requestScope.beginDate }" pattern="yyyy-MM-dd"/>&endDate=<fmt:formatDate value="${requestScope.endDate }" pattern="yyyy-MM-dd"/><c:if test="${not empty internal}">&internal=${internal }</c:if><c:if test="${not empty requestScope.gameId}">&gameId=${requestScope.gameId }</c:if>" />
			...
			<tr height="30px">
				...
				<form name="pageForm" action="getPlayerItems.do" method="post">
		...
		<br />
		<div align="center">
			<input type="button" value="<bean:message bundle="pageResources" key="page.common.back"/>" onclick="window.location.href='showPlayerItems.do<c:if test="${not empty internal}">?internal=${internal }</c:if>'"/>
		</div>
		<div align="center">
			<font color="red" size="3px"><html:errors/> </font>
			<font color="red" size="3px">${msg }</font>
