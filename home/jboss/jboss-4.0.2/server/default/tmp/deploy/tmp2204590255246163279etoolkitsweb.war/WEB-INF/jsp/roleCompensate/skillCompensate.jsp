...
<html>
	...
	<body bgcolor="#ffffff">
		...
		<form name="pageForm" action="${requestScope.operType }.do" method="post">
			<table width="90%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr height="30px">
					<td colspan="7" align="left">
						<gmweb:createCompenTitleButton operType="${requestScope.operType}" actionName="roleCompensate" methodType="SkillCompensate" />
					</td>
				</tr>
				<tr>
				...
				<c:forEach items="${requestScope.pageInfo.infoList}" var="skillCompensate" varStatus="status">
					<tr height="23px">
						<td width="10%" align="center" class="t_border_l">
							<input name="compensateId" type="checkbox" value="${skillCompensate.COMPENSATE_ID }" onclick="uncheckAll(document.forms[0]);" <c:if test="${requestScope.apprType != 'batchFirstApprSkillCompensate' and requestScope.apprType != 'batchSecondApprSkillCompensate' }">disabled</c:if> />
						</td>
						<td align="center" class="t_border_l">
							${status.index + 1 }
						</td>
						<td align="center" class="t_border_l">
							${skillCompensate.OPERATOR_NAME }
						</td>
						<td align="center" class="t_border_l">
							<fmt:formatDate value="${skillCompensate.INSERT_TIME }"
								pattern="yyyy-MM-dd HH:mm:ss" />
						</td>
						<td align="center" class="t_border_l">
							${skillCompensate.REASON }
						</td>
						<td align="center" class="t_border_l">
							<gmweb:approvalResult2Text approvalResult="${skillCompensate.APPROVAL_RESULT}" />
						</td>
						<td align="center" class="t_border_r">
							<gmweb:createCompenOperLink operType="${requestScope.operType}" actionName="roleCompensate" methodType="SkillCompensate" compensateId="${skillCompensate.COMPENSATE_ID}"/>
						</td>
					</tr>
				</c:forEach>
				<tr height="30px">
			...
			<c:if test="${requestScope.operType == 'showSkillCompensate' }">
			<tr height="30px">
				<td colspan="7" align="left">
					<input type="button" value="<bean:message bundle="pageResources" key="page.roleCompensate.awardCompensate.createAwardCompensate"/>" onclick="window.location.href='showCreateSkillCompensate.do'" />
				</td>
			</tr>
			</c:if>
			<c:if test="${requestScope.apprType == 'batchFirstApprSkillCompensate' or requestScope.apprType == 'batchSecondApprSkillCompensate' }">
			<tr height="30px">
				<td colspan="7" align="left">
