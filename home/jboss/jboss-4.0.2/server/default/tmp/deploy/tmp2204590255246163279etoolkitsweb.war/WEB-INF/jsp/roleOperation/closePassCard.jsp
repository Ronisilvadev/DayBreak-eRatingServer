...
<html>
	...
	<body bgcolor="#ffffff">
		...
		<form name="pageForm" action="${requestScope.operType }.do" method="post">
			<table width="90%" border="0" cellpadding="0" cellspacing="0"
				...
				<tr height="30px">
					<td colspan="8" align="left">
						<gmweb:createCompenTitleButton operType="${requestScope.operType}" actionName="roleOperation" methodType="ClosePassCard" />
					</td>
				</tr>
				<tr>
				...
				<tr height="23px" class="h_CDE4F6">
					...
					<td align="center" class="t_border_l">
						<bean:message bundle="pageResources" key="page.roleOperation.operModifyBindEmail.proposer"/>
					</td>
					<td align="center" class="t_border_l">
				...
				<c:forEach items="${requestScope.pageInfo.infoList}" var="closePassCard" varStatus="status">
					<tr height="23px">
						<td align="center" class="t_border_l">
							<input name="compensateId" type="checkbox" value="${closePassCard.changeId}" onclick="uncheckAll(document.forms[0]);" 
							<c:if test="${requestScope.operType != 'showFirstApprClosePassCard' and requestScope.operType != 'showSecondApprClosePassCard' }">disabled</c:if>></input>
						</td>
						<td align="center" class="t_border_l">
							${status.index + 1 }
						</td>
						<td align="center" class="t_border_l">
							<c:out value="${closePassCard.passportName }"/>
						</td>
						<td align="center" class="t_border_l">
							${closePassCard.operatorName }
						</td>
						<td align="center" class="t_border_l">
							<fmt:formatDate value="${closePassCard.insertTime }"
								pattern="yyyy-MM-dd HH:mm:ss" />
						</td>
						<td align="center" class="t_border_l">
							${closePassCard.reason }
						</td>
						<td align="center" class="t_border_l">
							<gmweb:approvalResult2Text approvalResult="${closePassCard.approvalResult}" />
						</td>
						<td align="center" class="t_border_r">
							 <gmweb:createCompenOperLink operType="${requestScope.operType}" actionName="roleOperation" methodType="ClosePassCard" compensateId="${closePassCard.changeId}"/>
						</td>
					</tr>
				</c:forEach>
				<tr height="30px">
			...
			<c:if test="${requestScope.operType == 'showClosePassCard' }">
			<tr height="30px">
				<td colspan="8" align="left">
					<input type="button" value="<bean:message bundle="pageResources" key="page.roleOperation.modifyBindEmail.createPropose"/>" onclick="window.location.href='showCreateClosePassCard.do'" />
				</td>
			</tr>
			</c:if>
			<c:if test="${requestScope.operType == 'showFirstApprClosePassCard' or requestScope.operType == 'showSecondApprClosePassCard' }">
			<tr height="30px">
				<td colspan="8" align="left">
					...
					<input name="apprButton" type="button" value="<bean:message bundle="pageResources" key="page.roleOperation.modifyBindEmail.allSubmit"/>" disabled onclick="apprSubmit(document.forms[0]);"/>
				</td>
			</tr>
			</c:if>
			</table>
